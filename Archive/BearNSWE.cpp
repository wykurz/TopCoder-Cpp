...
class BearNSWE {
public:
    double totalDistance(vector <int> dist, string dir)
    {
        int x = 0, y = 0;
        int n = dist.size();
        REP(i, n) switch (dir[i]) {
            case 'N': y += dist[i]; break;
            case 'S': y -= dist[i]; break;
            case 'E': x -= dist[i]; break;
            case 'W': x += dist[i]; break;
        }
        return accumulate(ALL(dist), 0) + sqrt(SQR(x) + SQR(y));
    }
};
// BEGIN CUT HERE
...
{
    {
        ...
        BearNSWE theObject;
        eq(0, theObject.totalDistance(a, "NES"),10.60555127546399);
    }
    {
        int aARRAY[] = {10,10,10,10};
        vector <int> a( aARRAY, aARRAY+ARRSIZE(aARRAY) );
        BearNSWE theObject;
        eq(1, theObject.totalDistance(a, "NWSE"),40.0);
    }
    {
        int aARRAY[] = {10,10,10,15,8,20,5};
        vector <int> a( aARRAY, aARRAY+ARRSIZE(aARRAY) );
        BearNSWE theObject;
        eq(2, theObject.totalDistance(a, "NEWEWWE"),90.8062484748657);
    }
    {
        int aARRAY[] = {42};
        vector <int> a( aARRAY, aARRAY+ARRSIZE(aARRAY) );
        BearNSWE theObject;
        eq(3, theObject.totalDistance(a, "E"),84.0);
    }
    {
        int aARRAY[] = {10,40,40};
        vector <int> a( aARRAY, aARRAY+ARRSIZE(aARRAY) );
        BearNSWE theObject;
        eq(4, theObject.totalDistance(a, "NSE"),140.0);
    }
    return 0;
}
// END CUT HERE
