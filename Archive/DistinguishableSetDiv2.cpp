...
{
    {
        string answerARRAY[] = {"AA","AB","CC"};
        vector <string> answer( answerARRAY, answerARRAY+ARRSIZE(answerARRAY) );
        DistinguishableSetDiv2 theObject;
        eq(0, theObject.count(answer),2);
    }
    {
        string answerARRAY[] = {"XYZ","XYZ","XYZ"};
        vector <string> answer( answerARRAY, answerARRAY+ARRSIZE(answerARRAY) );
        DistinguishableSetDiv2 theObject;
        eq(1, theObject.count(answer),0);
    }
    {
        string answerARRAY[] = {"AAAA","BACA","CDCE"};
        vector <string> answer( answerARRAY, answerARRAY+ARRSIZE(answerARRAY) );
        DistinguishableSetDiv2 theObject;
        eq(2, theObject.count(answer),11);
    }
    {
        string answerARRAY[] = {"HGHHGUHUHI","BQHJWOSZMM","NDKSKCNXND","QOEOEIWIDS","IIQIWUNNZM"};
        vector <string> answer( answerARRAY, answerARRAY+ARRSIZE(answerARRAY) );
        DistinguishableSetDiv2 theObject;
        eq(3, theObject.count(answer),1017);
    }
    {
        string answerARRAY[] = {"XYZ","XAB","YAC"};
        vector <string> answer( answerARRAY, answerARRAY+ARRSIZE(answerARRAY) );
        DistinguishableSetDiv2 theObject;
        eq(4, theObject.count(answer),5);
    }
    return 0;
}
// END CUT HERE
