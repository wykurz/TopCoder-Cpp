...
public:
    string makerope(string s)
    {
        ...
        vector<int> len;
        auto ss = split(s, ".");
        transform(ALL(ss), back_inserter(len), [](string& el){ return el.size(); });
        sort(ALL(len));
        reverse(ALL(len));
        vector<int> even;
        vector<int> odd;
        for (int l : len) if (l & 1) odd.push_back(l); else even.push_back(l);
        auto gets = [](int l) { string ans; for (int i = 0; i < l; ++i) ans += '-'; return ans; };
        string ans;
        for (int l : even) { ans += gets(l); ans += '.'; --space; }
        for (int l : odd) { ans += gets(l); ans += '.'; --space; }
        if (0 < even.size() + odd.size()) { ++space; ans = ans.substr(0, ans.size() - 1); }
        for (int i = 0; i < space; ++i) ans += ".";
        return ans;
    }
};
// BEGIN CUT HERE
...
{
    {
        Ropestring theObject;
        eq(0, theObject.makerope("..-..-"),"-.-...");
    }
    {
        Ropestring theObject;
        eq(1, theObject.makerope("-.-"),"-.-");
    }
    {
        Ropestring theObject;
        eq(2, theObject.makerope("--..-.---..--"),"--.--.---.-..");
    }
    {
        Ropestring theObject;
        eq(3, theObject.makerope("--..-.---..--..-----.--."),"--.--.--.-----.---.-....");
    }
    {
        Ropestring theObject;
        eq(4, theObject.makerope("..."),"...");
    }
    return 0;
}
// END CUT HERE
